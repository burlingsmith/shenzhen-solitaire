...
takeLimit : Stack -> Int
takeLimit stack =
    takeLimit_ Nothing stack 0

takeLimit_ : Maybe Card -> Stack -> Int -> Int
takeLimit_ lastCard remaining acc =
    case remaining of
        [] ->
            acc
        head::tail ->
            case lastCard of
                Nothing ->
                    takeLimit_ (Just head) tail (acc + 1)
                Just card ->
                    if
                        (Card.next card head) && not (Card.sameSuit head card)
                    then
                        takeLimit_ (Just head) tail (acc + 1)
                    else
                        acc
